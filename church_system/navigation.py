...
def _finance_sections(user, active_church=None):
    ...
    if can_view_ledger(user) and _church_feature(active_church, "ledger", user):
        ledger_items = [_item("General Ledger", "ledger:index", "bi-journal-text")]
    ...
    if treasury_items:
        sections.append(_section("Treasury", treasury_items, "treasury"))
    ...
    if can_view_reconciliation(user) or can_manage_reconciliation(user):
        planning_items.append(_item("Reconciliation", "transactions:reconciliation_list", "bi-bank"))
    if planning_items:
        sections.append(_section("Planning", planning_items, "planning"))
    ...
    if (
        ...
        if remittance_items:
            sections.append(_section("Remittance", remittance_items, "remittance"))
...
def _people_sections(user):
    ...
    if can_view_members(user) or can_manage_members(user):
        member_items.append(_item("Directory", "members:list", "bi-list-ul"))
    ...
    if can_manage_baptisms(user) or can_view_members(user):
        member_items.append(_item("Baptisms", "members:baptism_register", "bi-droplet"))

    sections = []
    if member_items:
        sections.append(_section("Members", member_items, "people"))
    ...
    if group_items:
        sections.append(_section("Groups & Care", group_items, "groups"))

    if can_view_meetings(user) or can_manage_meetings(user):
        meeting_items = [_item("All Meetings", "meetings:list", "bi-calendar-event")]
        ...
        sections.append(_section("Meetings", meeting_items, "meetings"))

    if can_view_member_records(user):
        sections.append(_section("Records", [
...
def _reports_sections(user, active_church=None):
    ...
    if _can_access_financial_tools(user):
        ...
        if can_view_budgets(user) or can_manage_budgets(user):
            financial_tools.append(_item("Budget vs Actual", "budgets:list", "bi-pie-chart"))
...
def get_main_navigation(user, active_church=None):
    ...
    if _can_access_people(user):
        ...
        if people_sections:
            nav.append(_dropdown("people", "People", "bi-people", sections=people_sections))
    ...
    if _can_access_communications(user):
        ...
        if can_create_announcements(user):
            publishing_items.append(_item("Create New", "announcements:create_announcement", "bi-pencil-square"))
        if publishing_items:
            comm_sections.append(_section("Publishing", publishing_items, "communications"))
