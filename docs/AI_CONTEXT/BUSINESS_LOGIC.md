...
`SUPER_ADMIN`, `GENERAL_OVERSEER`, `UNION_ADMIN`, `CONFERENCE_ADMIN`, `ZONE_DIRECTOR`, `DISTRICT_PASTOR` (District Administrator), `DISTRICT_TREASURY` (District Treasurer), `LOCAL_PASTOR`, `SECRETARY`, `TREASURY`, `BOARD_MEMBER`, `MEMBER`
...
| Paid upgrade request | Expired or ≤7 days to expiry → `/accounts/subscription-plans/` (active catalog + plan prices) → `/accounts/subscription-pay/` (interval + “I have paid”) then `submit_activation_request` (institution + payment reference + optional receipt, snapshots `requested_plan`/amount). Duplicate payment references across churches are rejected. Notify platform operators; activate only via `record_subscription_payment` (applies requested plan/interval). Record-payment GET prefills reference and paid-at from the open activation request. |
...
- `dashboard`: KPIs, notifications (`Notification`), church switching for hierarchy users. **Current:** `DashboardScope` is `SUBTREE` by default for hierarchy users with more than one church until they select a congregation (`get_active_church` is `None` in that case so the toolbar shows All churches). Money KPI cards and the 12-month financial trend chart use the **open working day** (else last closed day) as the posting month when a church is focused — not the server calendar month — so a January working day shows January MTD even if the clock is in September. Money KPI cards require post/approve/receipt/cutoff permission, except District Administrator / District Treasurer who use `view_dashboard_finance` over their district subtree. Default `manage_finances` is treasury/union/zone/local pastors — not SECRETARY and not district roles; clerks keep receipts/expenses. District Administrator sees finance summaries (transactions, remittance, finance reports) without posting, approving, reconciling, or GL/giving detail. District Treasurer has those reads plus ledger, giving, recon worksheets, payroll summaries, and exports — still without default posting/approve. Local church home is operational (receipts, approvals, remittance, members); subtree home is roll-up + `get_scope_exception_board`. Church-focused KPI cards include Net MTD, Expenses MTD, and Combined; Net MTD and Expenses MTD open the monthly Financial Summary report (`?period=monthly`), not receipt entry. The This-week pulse hides zero chips and the strip itself when nothing is due. Member portal prayer actions sit beside an Announcement button. Admins receive an in-app notice at most once per week when manageable churches have no current-year church-level budget. Secretaries and other users with `view_members` / `manage_members` get `membership_analysis`: an active-status member comparison chart (church / district / conference switches, scoped to `DashboardScope.church_ids`). Grouping (`member_view`) and district/conference filters persist on the query string; with six or more churches across more than one district the chart defaults to Districts. Church axis labels are abbreviated; the full name is drawn on the bar, and a single-district church view shows each congregation’s share of that district. Clicking a district or conference bar drills into that filter. Church-focused home hides Workbench/Evidence kickers and leaderboards; compact member KPIs share the same card language as money KPIs. Home shows a compact `member_dashboard` (total/active/inactive/new this month) with a link to the Member Summary report for charts, demographics, activity, and data quality. The home financial trend chart switches Tithe / Combined / Income / Expense / Income vs expense (`finance_chart` query param; default Tithe). Single series show monthly bars plus a cumulative line (12 posting months). Compare shows grouped income and expense bars with a monthly net line. Workspace Net MTD is omitted on `/dashboard/` so it is not duplicated with the cards. Remittance overdue / pending / transferred / settlement drafts appear as clickable home alert banners only when those counts are non-zero (not a persistent zero strip). `build_kpi_widgets` follows role `WIDGET_ORDER` only; `dashboard/home_panels.py` for inbox, remittance alerts, membership analysis, activity feed, coaching hints, portal alerts.
