...
- After cutoff, church users use `/accounts/subscription-expired/` → `/accounts/subscription-pay/` (complete payment, confirm, choose monthly/yearly) → `/accounts/subscription-subscribe/` (institution + payment reference + optional receipt, no email). Duplicate payment references across churches are rejected. That creates `SubscriptionActivationRequest` (`PENDING`) and immediately creates in-app `Notification` rows for every active `is_platform_user`. Institution users whose `expires_at` is 1–7 days away see an in-app banner and one dashboard notification. Operators see a Control Room alert, a top-bar count, and `/platform/activation-requests/` (plan, amount, church code). Paid access is still turned on with **Record payment** on the tenant subscription (which marks matching requests `ACTIVATED`).
