...
Institution prefixes include `/dashboard/`, `/members/`, `/organization/`, `/transactions/`, `/permissions/`, `/announcements/`, `/reports/`, `/meetings/`, `/budgets/`, `/giving/`, `/contributions/`, `/controls/`, `/approvals/`, `/intelligence/`, `/ledger/`, `/remittance/`, `/payroll/`, `/assets/`, `/portal/`.
...
- After cutoff, church users use `/accounts/subscription-expired/` → `/accounts/subscription-plans/` (all active `SubscriptionPlan` rows with monthly/yearly prices) → `/accounts/subscription-pay/` (complete payment, confirm, choose monthly/yearly for the selected plan) → `/accounts/subscription-subscribe/` (institution + payment reference + optional receipt, no email). Duplicate payment references across churches are rejected. That creates `SubscriptionActivationRequest` (`PENDING`, with `requested_plan` and amount snapshot) and immediately creates in-app `Notification` rows for every active `is_platform_user`. Institution users whose `expires_at` is 1–7 days away see an in-app banner and one dashboard notification. Operators see a Control Room alert, a top-bar count, and `/platform/activation-requests/` (plan, amount, church code). Paid access is still turned on with **Record payment** on the tenant subscription (which assigns the requested plan/interval and marks matching requests `ACTIVATED`). The record-payment form prefills payment reference and paid-at from the church’s pending/acknowledged `SubscriptionActivationRequest`.
