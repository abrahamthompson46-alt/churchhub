...
| `SubscriptionActivationRequest` | UUID | FK church CASCADE; optional subscription/denomination; optional requested_plan SET_NULL; payment_reference + normalized unique (non-rejected); billing_interval/amount/currency/plan_name; optional receipt; status PENDING/ACKNOWLEDGED/ACTIVATED/REJECTED; unique pending per church |
